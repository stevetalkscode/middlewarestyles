...
public class ConventionalGuidMiddlewareByConstructor
{
    ...
    {
        context.Items.Add($"{GuidInstance.ContextKey}_{nameof(ConventionalGuidMiddlewareByConstructor)}", $"By Convention Constructor Injection : {_guidInstance} - CAPTURED as injected into singleton even though scoped");
            await _next(context);
    }
}
}